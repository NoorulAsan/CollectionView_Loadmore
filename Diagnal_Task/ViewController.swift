...
class ViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    @IBOutlet var view_Loader: UIView!
    @IBOutlet var indicator_Loader: UIActivityIndicatorView!
    

    override func viewDidLoad() {
        super.viewDidLoad()
        
        view_Loader.alpha = 0.0
        indicator_Loader.stopAnimating()
    ...
    func collectionView(_ collectionView: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        ...
        if indexPath.row == lastElement  && (isList2Loaded == false || isList3Loaded == false) {
            view_Loader.alpha = 1.0
            indicator_Loader.startAnimating()
            DispatchQueue.main.asyncAfter(deadline: .now() + 2) {
                self.view_Loader.alpha = 0.0
